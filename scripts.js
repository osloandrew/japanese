...
// from Norwegian. Captured once, here, before anything can call pushState
// (see updateURL()'s pretty-path navigation and anchorAppNavigationLinks()
// below): a real <base> tag pins this correctly on a captured static page
// regardless of how many navigations follow, but the plain app shell has no
// <base> tag, so APP_ROOT_URL falls back to document.URL -- which pushState
// *does* change. Reading it fresh on every call instead would let the first
// pretty-path pushState permanently move "the root" for every navigation
// after it.
const APP_ROOT_URL = document.baseURI;
let results = [];
// Runtime-only readiness state, ported from Norwegian. Signals to
// scripts/check-accessibility.py (via window.__APP_READY__ === true) that a
// route has finished loading and rendering. Keep this off the DOM so
// captured static pages cannot serialize a stale "ready" marker into their
// HTML.
...
// fallback is always correct in the meantime. Ported from Norwegian.
...
// on a freshly served page, though: once updateURL() has pushState'd to a
// pretty path (a word, a story, a feature route -- see STATIC_FEATURE_ROUTES
// below), there's no <base> tag here to anchor them the way a captured
// static page has, so the browser would resolve "?type=words" or "./"
// against that pretty path instead of the site root. Rewriting each href to
// an absolute, APP_ROOT_URL-anchored URL once here -- before any pushState
// has run -- sidesteps that entirely. Ported from Norwegian.
...
// Matches the slug rule scripts/capture-word-pages.py and
// capture-story-pages.py use for word/<slug>/ and story/<slug>/ directory
// names (and that make-sitemap.py's page-manifest.json already lists), so a
// pretty URL built here always names a page that actually exists on disk.
// Verified byte-for-byte against every real word/story in the CSVs: \p{L}/
// \p{N} treat kana and kanji as letters/numbers the same way Python's
// str.isalnum() does, so no Japanese-specific handling is needed beyond
// what Norwegian's version already does for Latin script. Ported from
// Norwegian's slugifyWordForURL().
...
function updateURL(query, type, selectedPOS, story = null, word = null) {
  ...
  // are the same thing, but on a pretty static page like /word/tシャツ/ (see
  // scripts/capture-word-pages.py), window.location.pathname would be
  // "/word/tシャツ/" and every call below would build a URL stacking query
  // params onto that path instead of the app's real root. Any interactive
  // navigation belongs at the app root regardless of which static page it
  // was triggered from. Ported from Norwegian's updateURL().
  ...
  if (primaryWord) {
    ...
    if (metadataEntry) {
      ...
      document.title = `${primaryWord} - Japanese Dictionary`; // Set title to the word
    }
  ...
  const prettyStoryURL =
    story && pageManifest.stories.has(slugifyWordForURL(story))
      ? new URL(`story/${slugifyWordForURL(story)}/`, appRootURL)
      : null;
...
// form (e.g. "cd", "あいにく", "道順を尋ねる") -- an alternate spelling or
// exact original case is already lost by the time it's a URL segment.
// Passing that straight to renderWordDefinition()/displayStory() only works
// by accident for the (common, but not universal) case where a word's
// slug is identical to itself; anything else needs the real text looked up
// by re-slugifying each candidate and matching. Returns the raw slug
// unchanged if no candidate matches (or candidates aren't loaded yet) -- the
// caller's existing "not found" handling covers that the same as before
// this path fallback existed. Ported from Norwegian's resolveSlugToText().
...
function loadStateFromURL() {
  ...
  const queryStoryTitle = url.searchParams.get("story"); // Check for a specific story parameter
  const queryWord = url.searchParams.get("word"); // Check for a specific word entry
  ...
  // capture-word-pages.py/capture-story-pages.py (e.g. /word/cd/,
  // /story/道順を尋ねる/) as equivalent to their ?word=/?story= query-string
  // form. A captured page already has its exact render baked in, so
  // resolving and re-rendering here is redundant there (but harmless --
  // renderWordDefinition()/displayStory() reproduce it exactly). It is NOT
  // redundant after updateURL()'s pushState navigates to this same pretty
  // URL in-app: the following back/forward button re-runs this function
  // with no preloaded globals and no query params, so resolving the slug
  // here is the only way that round-trips correctly. Query-string params
  // still win if present. Ported from Norwegian's parsePathState().
  const normalizedPath = url.pathname.replace(/\/index\.html$/, "/");
  const wordSlugMatch = normalizedPath.match(/\/word\/([^/]+)\/?$/);
  const storySlugMatch = normalizedPath.match(/\/story\/([^/]+)\/?$/);
  const pathWordSlug = wordSlugMatch
    ? decodeURIComponent(wordSlugMatch[1])
    : null;
  const pathStorySlug = storySlugMatch
    ? decodeURIComponent(storySlugMatch[1])
    : null;
  ...
  // ?story= param to read. Preferred over the slug resolution below when
  // present: it's the exact data the capture step rendered from, and is
  // available even before storyResults has loaded.
  const preloadedStory = window.__PRELOADED_STORY__;

  // A path-sourced title is a slug ("道順を尋ねる"), not guaranteed to be
  // the real display text -- most Japanese titles happen to slugify to
  // themselves, but this still resolves it the same way a word does rather
  // than assuming so. storyResults may not be loaded yet this early (its
  // fetch is sequenced by stories.js independently of the dictionary load
  // this function is triggered by); resolveSlugToText degrades to the raw
  // slug if so, same as if this path fallback didn't exist.
  const storyTitle = queryStoryTitle
    ? decodeURIComponent(queryStoryTitle)
    : !preloadedStory && pathStorySlug
      ? resolveSlugToText(
  ...
  if (storyTitle || preloadedStory) {
    const titleJapanese = storyTitle || preloadedStory.titleJapanese;
    ...
    // Norwegian sets this same flag for its story route from a separate
    // "initialStoryRoute" bypass in window.onload, because its stories.js
    // intercepts any ?story= URL before loadStateFromURL() ever runs. This
    // app's stories.js only intercepts the captured /stories/ list page
    // (see its DOMContentLoaded's isPrettyStoriesListPath check) -- a
    // query-string story route like ?type=stories&story=... genuinely
    // reaches this branch, so this is the equivalent point to mark ready.
    window.__APP_READY__ = true;
    return; // Exit function as story is being displayed
  }

  const word = queryWord || pathWordSlug; // resolved below once `results` is confirmed loaded
  ...
  function displayWordIfLoaded() {
    if (results.length > 0) {
      ...
      if (word) {
        // A path-sourced word is a slug ("cd"), not the real display text
        // ("CD") -- results is guaranteed loaded here, so this always
        // resolves correctly (see resolveSlugToText above).
        ...
        document.title = `${resolvedWord} - Japanese Dictionary`;
        syncModeNav("words");
        showLandingCard(false);
        resultsContainer.innerHTML = "";
